...
- **Mermaid HTML labels**: node labels using inline HTML such as `<br/>` and `<b>` now render correctly in the PDF export and preview. Previously the Mermaid source was injected unescaped, so the browser parsed those tags and they were dropped before rendering, joining lines together (e.g. `Mark Parise<br/>CEO` became `Mark PariseCEO`).
